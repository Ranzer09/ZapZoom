    ...
    const saveCart = async (cartData) => {
        try {
            ...
            if (!cartData || !Array.isArray(cartData.cart)) {
                throw new Error('Invalid cart data: items is undefined or not an array');
            }
    ...
    try { 
        ...
        if (!cart) {
            ...
            products.forEach(item => {
                ...
                } else {//check the contents properllyyy
        ...
        cart.total_price = cart.cart.reduce((total, product) => total + (product.price * product.qty), 0);
        cart.total_qty = cart.cart.reduce((total, product) => total + product.qty, 0);
...
const clearCart = async (req, res) => {
    ...
    try {
        if(!mongoose.Types.ObjectId.isValid(email))//check for id validity
        return res.status(404).json({error:'Cart not found'})
